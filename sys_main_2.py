...
class Plot2D(QtWidgets.QMainWindow):
    ...
    def writeToCSV(self, time_diff, event, indicators, hw, time_after):
        # open csv
        print("############################\n\n\n\n")
        ...
        with open("hw_compliance_data.csv", "a") as f:
            ...
            l = [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), time_diff, event, indicators, hw, time_after, self.globals["HW_EVENTS"],
                 self.globals["COMPLETED_HW"]]
            writer = csv.writer(f)
            writer.writerow(l)
    ...
    def hw_isr(self, e):
        ...
        if (self.flags["TRIG"]):
            # if the device was triggered,
            print("Recording for recent event")
            self.globals["COMPLETED_HW"] += 1
            dif =  time.time() - self.globals["HW_TRIG_TIME"]
            self.writeToCSV(0, "entering", 1, 1, dif if dif <= 180 else ">3min")
        else:
            # if the device is not triggered, count new event as well
            print("recording as new record")
            self.writeToCSV(0, "IR", 0, 1, 0)
            self.globals["COMPLETED_HW"] += 1
    ...
    def update_adc_measurement(self):
        ...
        '''
        if (self.flags["DATA"] == False and np.abs(time.time() - self.globals["SUCCESS_TRIG"]) >= self.globals[
            "SUCCESS_TIMER_THRESH"]):
        ...
        if (e1 >= 3.75 or e2 >= 3.75):
            ...
            if (self.trigger_cnt >= 15):
                ...
                print("g1: {0} \n g2: {1} \n t1: {2} \n t2: {3}".format(s1_gr, s2_gr, self.first_trigger,
                                                                        self.last_trigger))

                _classify = impurity.classify(
                    [self.first_trigger, self.last_trigger, s1_gr, s2_gr, s1_p1, s1_p2, s2_p1, s2_p2],
                    self.tree_dir)
                max_guess = 0
                max_class = None
        ...
        if (dir != "right" and dir != "left"):
            ...
            if (dir == 'right' and self.flags['DIRECTION'] == 1 or dir == "left" and self.flags["DIRECTION"] == 0):
                ...
                if (np.abs(time.time() - self.globals["TRIG_TIME"]) >= self.globals["TRIG_THRESH"]
                    ...
                    print("Timer Flag: {}".format(
                        np.abs(time.time() - self.globals["TRIG_TIME"]) >= self.globals["TRIG_THRESH"]))
                ...
                print("Setting timer")
                self.writeToCSV(0, 'leaving',0,0,0)
